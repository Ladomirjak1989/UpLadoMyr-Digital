...
import React, { useEffect, useState } from 'react';
import { createPortal } from 'react-dom'; // ⬅︎ NEW
import Link from 'next/link';
import Image from 'next/image';
import { FiMenu, FiX, FiUser, FiChevronRight, FiChevronDown } from 'react-icons/fi';
import { usePathname } from 'next/navigation';
...
type MobileMenuProps = {
  ...
  isServicesOpenMobile: boolean;
  setIsServicesOpenMobile: React.Dispatch<React.SetStateAction<boolean>>;
};
...
const MobileMenu: React.FC<MobileMenuProps> = ({
  ...
  return createPortal(
    <>
      {/* overlay поверх ВСІЄЇ сторінки */} {/* ⬅︎ NEW */}
      <div
        ...
        aria-hidden={!isOpen}
      />
      {/* панель меню, теж фіксована від верху до низу */} {/* ⬅︎ NEW */}
      <nav
        className={`md:hidden fixed inset-y-0 right-0 w-[86%] max-w-[340px]
          transform transition-transform duration-300 ease-[cubic-bezier(.22,1,.36,1)]
          ${isOpen ? 'translate-x-0' : 'translate-x-full'}
          z-[140] bg-gradient-to-br from-[#f7f4ea] via-[#efe8d9] to-[#e0d5bf]`}
        aria-label="Mobile navigation"
      >
        <div className="h-full flex flex-col rounded-l-2xl border-l border-[#e6dcc8] shadow-2xl px-6 pt-6 pb-6 overflow-y-auto">
          <button
            onClick={onClose}
            className="text-[#1e3a8a] text-3xl absolute top-4 right-4"
            aria-label="Close menu"
            ...
            <li className="pt-3">
              <div className="rounded-xl border border-[#e7dcc9] bg-white/75 p-3 shadow">
                {isLoading ? (
                  ...
                  <AuthAction variant="logout" full onClick={onLogout} />
                ) : (
                  <AuthAction variant="login" full />
                )}
              </div>
...
const Header: React.FC = () => {
  ...
  // блокування скролу body, коли відкрите меню
  useEffect(() => {
  ...
  return (
    ...
    >
      ...
      {/* Мобільне меню рендериться ЧЕРЕЗ ПОРТАЛ */} {/* ⬅︎ NEW */}
      <MobileMenu
        isOpen={isOpen}
        ...
        isActive={isActive}
        isServicesOpenMobile={isServicesOpenMobile}
        setIsServicesOpenMobile={setIsServicesOpenMobile}
      />
      <FloatingSocialButtons isMenuOpen={isOpen} />
      <ScrollProgressBar
        placement="header"
