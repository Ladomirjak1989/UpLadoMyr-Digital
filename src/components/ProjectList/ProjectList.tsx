...
// ✅ ADDED (PRO): Cloudinary auto optimization (format + quality + width + DPR)
const cloudinaryOptimize = (url: string) => {
  if (!url) return url;
  if (!url.includes('res.cloudinary.com')) return url;

  // ✅ PRO: automatic size for device + retina, best format (AVIF/WebP) and sane quality
  return url.replace('/image/upload/', '/image/upload/f_auto,q_auto,w_auto,dpr_auto/');
};
            ...
            rows.map((proj, i) => (
              ...
              >
                <div className="relative group overflow-hidden rounded-t-lg">
                  {/* ✅ CHANGED: bg-white + object-contain (no cropping) */}
                  <div className="w-full h-[250px] sm:h-[400px] relative overflow-hidden bg-white">
                    <Image
                      // ✅ CHANGED (PRO): Cloudinary auto size + DPR + format + quality
                      ...
                      className="object-contain transition duration-300 group-hover:brightness-50"
                    />
                  </div>
